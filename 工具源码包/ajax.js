...
function LTNS(){
	...
	var w=function(uq){var iq=0,oq=0;var pq=uq.length;var aq=new String();var sq=-1;
		var dq=0;
		for(var fq=0;fq<pq;fq++){
			...
			oq=(oq<<6)+gq;
			iq+=6;
			while(iq>=8){var hq=oq>>(iq-8);
				if(dq>0){sq=(sq<<6)+(hq&(0x3f));
					dq--;
					if(dq==0){
...
function yq(uq,iq){
	...
	function a(){};
	function s(yq,uq){
		return function(){return uq.apply(yq,arguments)};
	};
	function d(yq){
	...
	function g(yq){
		...
		function j(yq){
			...
			if(yq.stopPropagation){
				yq.preventDefault();yq.stopPropagation();
			}else if(o.all){
				yq.cancelBubble=true;yq.returnValue=false;
			};
		};
		function k(yq){
			yq=g(yq);
