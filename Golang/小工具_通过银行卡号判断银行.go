...
func GetBankInfo(card string) (res string) {
	if card == "" || len(card) < 4 {
		res = "输入的银行卡卡号有误"
		return res
	}
	...
	headnum4 := card[:4]
	if maplist[headnum8] != "" {
		res = maplist[headnum8]
	} else if maplist[headnum6] != "" {
		res = maplist[headnum6]
	} else if maplist[headnum5] != "" {
		res = maplist[headnum5]
	} else if maplist[headnum4] != "" {
		res = maplist[headnum4]
	} else {
		res = "该卡号暂未录入"
	}
	return res
...
func main() {
	fmt.Println(GetBankInfo("6228481552887309119"))
}
