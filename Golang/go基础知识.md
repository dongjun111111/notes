...
// get file modified time 获取文件上次修改时间
...
// get file size as how many bytes 获取文件的大小/bytes
...
// get string from text file 将文本文件内容以字符串形式输出
...
// it returns false when it's a directory or does not exist.是否是文件
...
// IsExist returns whether a file or directory exists. 是否系统退出
...
func main() {
	//生成文件夹 以及 文件
	dir := "E:/test/"
	file, err := CreateFile(dir, "newdir")
	...
	//写文件
	content := "这是新建文件的内容"
	l, e := FilePutContent(file+"newfile.txt", content)
...
func main(){
	...
	fmt.Fprintf(os.Stdout,"telent port:%d",port)
...
##写在最后，发现markdown好像不能再显示更多的东西了，决定另开一个文件。