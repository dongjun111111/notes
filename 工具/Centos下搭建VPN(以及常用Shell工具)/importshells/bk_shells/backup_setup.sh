...
if [[ "$DESC_BK" =~ ^[3,5,6]$ ]];then 
    while :; do echo
        ...
        echo -e "\t${CMSG}1${CEND}. 华北 1 (青岛)         ${CMSG}2${CEND}. 华北 2 (北京)"
        echo -e "\t${CMSG}3${CEND}. 华东 1 (杭州)         ${CMSG}4${CEND}. 华东 2 (上海)"
        echo -e "\t${CMSG}5${CEND}. 华南 1 (深圳)         ${CMSG}6${CEND}. 香港"
        echo -e "\t${CMSG}7${CEND}. 新加坡                ${CMSG}8${CEND}. 美西 1 (硅谷)"
        echo -e "\t${CMSG}9${CEND}. 美东 1 (弗吉尼亚)"
        read -p "Please input a number:(Default 1 press Enter) " Locatin
    ...
    [ "$Location" == '1' ] && Host=oss-cn-qingdao-internal.aliyuncs.com
    [ "$Location" == '2' ] && Host=oss-cn-beijing-internal.aliyuncs.com
    [ "$Location" == '3' ] && Host=oss-cn-hangzhou-internal.aliyuncs.com
    [ "$Location" == '4' ] && Host=oss-cn-shanghai-internal.aliyuncs.com
    [ "$Location" == '5' ] && Host=oss-cn-shenzhen-internal.aliyuncs.com
    [ "$Location" == '6' ] && Host=oss-cn-hongkong-internal.aliyuncs.com
    [ "$Location" == '7' ] && Host=oss-ap-southeast-1-internal.aliyuncs.com
    [ "$Location" == '8' ] && Host=oss-us-west-1-internal.aliyuncs.com
    [ "$Location" == '9' ] && Host=oss-us-east-1-internal.aliyuncs.com
    ...
    while :; do echo
        ...
        ./tools/osscmd ls --id=$KeyID --key=$KeySecret >/dev/null 2>&1
        if [ $? -eq 0 ];then
