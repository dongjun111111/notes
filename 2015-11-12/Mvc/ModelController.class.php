...
class ModelController extends AdminController {
    ...
    public function edit(){
        ...
        /*获取一条记录的详细数据*/
        $Model = M('Model');
        $data = $Model->field(true)->find($id);                         //调用模型文件中find方法查询数据库
        if(!$data){
    ...
    public function del(){
        ...
        foreach ($ids as $value){
            $res = D('Model')->del($value);                                //调用模型文件中del方法,删除数据库
            if(!$res){
                break;
            }
        }
     ...
     * @author huajie <banhuajie@163.com>
     */
    public function update(){                                            //调用模型文件中的update方法，更新数据库
        $res = D('Model')->update();
    ...
    public function generate(){
        if(!IS_POST){
            ...
            $res = D('Model')->generate($table);                               //调用模型文件中generate方法插入数据库
            if($res){ 
